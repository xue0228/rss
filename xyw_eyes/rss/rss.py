...
class WriteXmlMixin:
    ...
    async def async_write(self, path: str, encoding: str = 'utf-8') -> None:
        """
        异步写入数据到xml文件
        :param path: 文件路径
    ...
    if isinstance(obj, str) or obj is None:
        ...
        if obj is not None:
            if name in ['title', 'description']:
                handler.ignorableWhitespace(cdata(obj))
            else:
...
def _format_date(dt):
    ...
    return "%s, %02d %s %04d %02d:%02d:%02d GMT+0800 (CST)" % (
...
class RSS2(WriteXmlMixin):
    ...
    def __init__(self,
                 ...
                 cloud=None,  # a Cloud
                 ttl=60,  # integer number of minutes
...
def cdata(content: str) -> str:
    return '<![CDATA[{}]]>'.format(content)


def img(url: str) -> str:
    return '<img src="{}" referrerpolicy="no-referrer">'.format(url)
...
def parse_string_to_datetime(date_str: str) -> datetime.datetime:
    # return parse(date_str) - datetime.timedelta(hours=8)
    return parse(date_str)


if __name__ == '__main__':
    url = 'https://cdn.zzdaye.com:8443//images//7//7//d//77d00ef0-077a-11ea-9095-01042116c660.jpg'
    print(cdata(url))
    print(img(url))
